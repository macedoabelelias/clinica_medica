...
<td>
	<big><a class="btn btn-info btn-sm {$inserir_orcamentos}" href="#" onclick="editar('{$id}','{$cliente}','{$tipo}','{$dias_validade}','{$desconto}','{$tipo_desconto}','{$forma_pgto}','{$usuario}','{$obs}')" title="Editar Dados"><i class="fa fa-edit "></i></a></big>
...
<big><a class="{$ocultar} btn btn-success btn-sm" href="#" onclick="baixarOrc('{$id}')" title="Aprovar Orçamento"><i class="fa fa-check-square "></i></a></big>
		...
		<big><a class="btn btn-dark-light btn-sm" href="#" onclick="arquivo('{$id}', '{$nome_cliente}')" title="Inserir / Ver Arquivos"><i class="fa fa-archive" ></i></a></big>

		

			<form   method="POST" action="rel/orcamento_class.php" target="_blank" style="display:inline-block">
					<input type="hidden" name="id" value="{$id}">
					<big><button class="btn btn-primary btn-sm" title="Listar Orçamento"><i class="fa fa-file-pdf-o "></i></button></big>
					</form>
...
<script type="text/javascript">
	function editar(id, cliente, tipo, dias_validade, desconto, tipo_desconto,  forma_pgto, usuario, obs){
		$('#mensagem').text('');

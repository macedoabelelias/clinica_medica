...
if($nivel_usuario == 'Administrador'){
	$query = $pdo->query("SELECT * from $tabela where nivel != 'Cliente' order by id desc");
}else{
...
<td>
	<a class="btn btn-info-light btn-sm" href="#" onclick="editar('{$id}','{$nome}','{$email}','{$telefone}','{$endereco}','{$nivel}','{$pix}','{$data_nasc}','{$numero}','{$bairro}','{$cidade}','{$estado}','{$cep}','{$acessar_painel}','{$complemento}','{$tipo_chave}', '{$foto}', '{$comissao}', '{$atendimento}', '{$intervalo}', '{$cro}')" title="Editar Dados"><i class="fa fa-edit "></i></a>

<a href="#" class="btn btn-danger-light btn-sm" onclick="excluir('{$id}')" title="Excluir"><i class="fa fa-trash-can "></i></a>

<a class="btn btn-primary-light btn-sm" href="#" onclick="mostrar('{$nome}','{$email}','{$telefone}','{$endereco}','{$ativo}','{$dataF}', '{$senha}', '{$nivel}', '{$foto}','{$pix}','{$data_nasc}','{$numero}','{$bairro}','{$cidade}','{$estado}','{$cep}','{$acessar_painel}','{$complemento}','{$tipo_chave}', '{$comissao}', '{$atendimento}', '{$intervalo}', '{$cro}')" title="Mostrar Dados"><i class="fa fa-info-circle "></i></a>
...
<script type="text/javascript">
	function editar(id, nome, email, telefone, endereco, nivel, pix, data_nasc, numero, bairro, cidade, estado, cep, acessar_painel, complemento, tipo_chave, foto, comissao, atendimento, intervalo, cro) {
		$('#mensagem').text('');
	...
	function mostrar(nome, email, telefone, endereco, ativo, data, senha, nivel, foto, pix, data_nasc, numero, bairro, cidade, estado, cep, acessar_painel, complemento, tipo_chave, comissao, atendimento, intervalo, cro ) {
		...
		$('#intervalo_dados').text(intervalo);
		$('#cro_dados').text(cro);
